...
class AdminDashboardController extends Controller
{
    public function index()
    {
        ...
        for ($i = 1; $i <= 12; $i++) {
            ...
            $monthlyLabels[] = $date->translatedFormat('F Y'); 
            
            $lodgingMonthlyCounts[] = Lodging::whereYear('created_at', $date->year)
                ->whereMonth('created_at', $date->month)->count();
                
            $hangoutMonthlyCounts[] = HangoutPlace::whereYear('created_at', $date->year)
                ->whereMonth('created_at', $date->month)->count();
                
            $touristMonthlyCounts[] = TouristPlace::whereYear('created_at', $date->year)
                ->whereMonth('created_at', $date->month)->count();
        }
